...
安装 Node.js 24 和 pnpm 10，执行 `pnpm install --frozen-lockfile`，然后 `pnpm dev`。正式构建为 `pnpm build`，输出在 dist。

## 发布和后续修改

在 GitHub 仓库 Settings → Pages 中将 Source 设为 GitHub Actions。推送 main 后，Publish website 工作流自动发布。之后可继续让 Codex 修改此仓库、检查并推送更新。
